...
MNIST_data_path = r"C:\Users\wangbinqiang\sinabs\data\MNIST_Dataset\raw"
...
def get_labeled_data(picklename, bTrain = True):
    ...
    if os.path.isfile('%s.pickle' % picklename):
        ...
        if bTrain:
            images = open(MNIST_data_path + '\\train-images-idx3-ubyte','rb')
            labels = open(MNIST_data_path + '\\train-labels-idx1-ubyte','rb')
        else:
            images = open(MNIST_data_path + '\\t10k-images-idx3-ubyte','rb')
            labels = open(MNIST_data_path + '\\t10k-labels-idx1-ubyte','rb')
...
neuron_eqs_e += '\n  dtimer/dt = 100.0  : second' # there maybe error becased of ms to second
...
neuron_groups['i'] = b.NeuronGroup(n_i*len(population_names), neuron_eqs_i, threshold= v_thresh_i, refractory= refrac_i, reset= v_reset_i) #compile = True, 
...
for name in population_names:
    ...
    neuron_groups[name+'e'] = neuron_groups['e'][:n_e] #.subgroup(n_e)
    neuron_groups[name+'i'] = neuron_groups['i'][:n_i]   #.subgroup(n_i)
    
    neuron_groups[name+'e'].v = v_rest_e - 40. * b.mV
    neuron_groups[name+'i'].v = v_rest_i - 40. * b.mV
    if test_mode or weight_path[-8:] == 'weights/':
        neuron_groups['e'].theta = np.load(weight_path + 'theta_' + name + ending + '.npy')*b.volt
    else:
    ...
    if record_spikes:
        spike_monitors[name+'e'] = b.SpikeMonitor(neuron_groups[name+'e'], record=False)
        spike_monitors[name+'i'] = b.SpikeMonitor(neuron_groups[name+'i'], record=False)
...
for name in input_connection_names:
    ...
    for connType in input_conn_names:
        ...
        connections[connName] = b.Synapses(input_groups[connName[0:2]], neuron_groups[connName[2:4]], 'w: 1', on_pre = 'g'+connType[0]+'+=w')
        connections[connName].connect()
        connections[connName].w = weightMatrix.flatten()
...
for i,name in enumerate(input_population_names):
    print(input_groups[name+'e'].rates.shape)
